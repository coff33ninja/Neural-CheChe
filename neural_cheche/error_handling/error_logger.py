...
class ErrorLogger:
    ...
    def log_error(self, 
        ...
        include_traceback = include_traceback if include_traceback is not None else self.config.get('include_traceback', True)
        include_context = include_context if include_context is not None else self.config.get('include_context', True)
    ...
    def _add_to_recent_errors(self, log_entry: Dict[str, Any]):
        ...
        self.error_counts[f"component_{component}"] = self.error_counts.get(f"component_{component}", 0) + 1
    ...
    def _check_log_rotation(self):
        ...
        # Check text log file
        if self.error_log_file.exists() and self.error_log_file.stat().st_size > max_size:
            self._rotate_log_file(self.error_log_file)
        
        # Check JSON log file
        if self.error_json_file.exists() and self.error_json_file.stat().st_size > max_size:
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        try:
            if self.summary_file.exists():
                ...
                return {'total_errors': 0, 'error_categories': {}, 'error_components': {}}
        except Exception as e:
